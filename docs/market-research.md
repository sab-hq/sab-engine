...
## 9. RMM/PSA/MCP Connector Landscape (August 2026)

*Source: a dedicated connector-landscape research pass conducted August 2026 (full report: [research artifact](https://claude.ai/public/artifacts/48ba565a-02a8-4d19-9244-b99042343383)), synthesizing vendor developer documentation and 2026 industry coverage. This section is narrower in scope than Sections 1–8 above — it's specifically about which existing tools SAB should integrate/partner with (RMM, PSA, patch/config management, orchestration platforms, and the MCP ecosystem) and in what order, rather than general market validation.*

**RMM platforms.** NinjaOne has the cleanest developer story for SAB's purposes: OAuth 2.0, a REST v2 API covering devices, ticketing, webhooks, and knowledge base/org documents, official webhook support, and a formal Technology Alliances Program (TAP). Multiple community MCP servers already exist for it. Datto RMM (Kaseya), Atera, and Syncro all have workable but more limited APIs — notably, Atera gates webhooks to Enterprise/Superpower tiers and disables its API during trials, which raises real evaluation friction for anyone trying to build against it early. N-able has moved aggressively into MCP, shipping an official MCP server on top of its GraphQL API with three tiered endpoints (read-only, full read/write, preview) — a pattern directly relevant to SAB's own recommend-and-approve posture (see below).

**PSA/ticketing platforms.** ConnectWise Manage has the deepest API and the most mature community MCP servers, but certified listing requires ConnectWise Invent, which includes a mandatory independent security review. Autotask PSA notably offers a free API-user license and simple header-key auth (no OAuth2 required) — a genuinely lower-friction entry point than ConnectWise. HaloPSA has a rich MCP ecosystem already but no native outbound webhooks (polling or its internal workflow engine required instead). ServiceNow shipped an official MCP Server Console (GA in its Zurich release) with OAuth 2.1/PKCE and formal AI governance tooling — enterprise-grade, but heavier than SAB's early-stage needs call for.

**IT documentation platforms — the closest comparables to `sab-kb`.** Hudu is the clear technical bar-setter: a modern REST API, scoped API keys, built-in webhooks, and — notably — an official, native OAuth-based MCP server shipped as a first-class product feature, not a community add-on. IT Glue (Kaseya) has a larger installed base but a materially weaker story: API-key-only auth with no OAuth, limited webhooks, and only unofficial/low-adoption community MCP servers. One widely-cited security critique of IT Glue (Kelvin Tegelaar/CyberDrain) notes its API key model is all-or-nothing — a leaked key exposes the entire documentation set, which is a real design lesson for how `sab-kb` should scope its own API keys.

**Enterprise patch/config management.** For `sab-engine`'s patching use case specifically, Ansible Automation Platform / AWX is the most credible execution substrate to orchestrate on top of rather than compete with — REST API, job templates, RBAC, and proven Windows Server patching support, with AWX itself open source. This reinforces the WSUS-integration approach already established in Section 6 of the design doc: SAB doesn't need to reimplement remote execution from scratch.

**Orchestration platforms as architectural precedent.** Rundeck's plugin ecosystem (1,400+ plugins, spanning Node Executors, File Copiers, and workflow steps in whatever language fits) is the clearest model for how SAB's own module/connector contracts (Section 4.2/4.4 of the design doc) could scale to a real community, and it already ships an official Ansible plugin — further evidence that "orchestrate Ansible rather than replace it" is a well-trodden, credible path.

**The MCP ecosystem, and where the gap is.** MCP has become the de facto standard for AI-to-tool connectivity in 2026 across RMM/PSA/ITSM vendors. The one repeatedly-cited gap across multiple sources: knowledge base / documentation MCP remains the largest *unmet* demand in the ecosystem — directly favorable to `sab-kb`'s positioning, and worth citing alongside this document's existing "brain debt" research (Section 1). Separately, Kaseya's 2026 State of the MSP Report (1,061 MSPs surveyed) found 48% of MSPs rank AI/automation as their clients' #1 need for 2026 — ahead of security (42%) and backup (36%) — yet only 13% currently generate meaningful revenue from it; CloudRadial's 2026 State of AI report separately found documentation-automation adoption sitting at just 15–24% of surveyed MSPs. Together these numbers give `sab-kb`'s core bet real, current-dated market backing rather than just intuition.

**Security findings that reinforce Section 2's recommend-and-approve principle.** A benchmark of 45 live MCP servers (MCPTox, arXiv:2508.14925, AAAI 2026 track) found tool-poisoning and prompt-injection attacks succeed at an average 36.5% rate, peaking at 72.8% for one model — and notably, more capable models were not more resistant, with refusal rates under 3% across the board. Separately, an April 2026 security disclosure (OX Security) described a by-design remote-code-execution flaw in the MCP STDIO transport spanning an estimated ~150 million SDK downloads and up to ~200,000 vulnerable server instances; Anthropic characterized the underlying behavior as expected rather than a bug to be patched at the protocol level. **Relevance to SAB:** this is a direct, current-dated, third-party validation of exactly the risk category Section 2's recommend-and-approve principle already anticipates — worth stating in SAB's own security messaging (Section 7 of the design doc) with the same explicit, prominent framing already used for the Section 2 trust-barrier research above, rather than treating MCP security as a separate, lesser concern.
...
| Recommend-and-approve as a security control, not just a trust-building UX choice | Section 9's MCP security findings (36.5% average tool-poisoning success rate, ~200,000 vulnerable server instances disclosed in 2026) are a direct, current, third-party validation that this isn't a hypothetical risk category |
| `sab-kb`'s knowledge-base focus as a market bet | Section 9 confirms knowledge base/documentation MCP is the most-cited unmet demand in the 2026 MCP ecosystem, and Kaseya's 2026 MSP survey data shows real, currently-unmet AI/automation demand from MSPs specifically |
| Orchestrating Ansible/AWX for `sab-engine`'s execution layer rather than building remote execution from scratch | Section 9 confirms AWX is a proven, open-source, API-driven substrate already used for Windows Server patching at scale — consistent with the "orchestrate what exists" philosophy already validated via Kestra/Rundeck above |
