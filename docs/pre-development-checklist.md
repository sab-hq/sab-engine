...
| PD-5 | Implement the claim/lease concurrency pattern for stateless workers (Section 4.1's concurrency model) | Extends the state machine (PD-4) to support more than one worker safely | 🟡 In Progress — **`WorkflowRunClaimService` written in `SabEngine.Orchestration`; implementation unchanged, test strategy fixed, not yet re-verified.** `TryClaimNextAsync` atomically claims the oldest eligible run via `ExecuteUpdateAsync` with a `WHERE` clause re-checked at write time (not just at the initial SELECT) — that's what actually prevents two workers from both winning the same run. Claimable states are `Requested`, `Declined`, `Approved`, `Failed`. **Confirmed real finding from Brock's first test run:** EF Core's InMemory provider throws `InvalidOperationException` ("could not be translated") on `ExecuteUpdateAsync` — exactly the risk flagged before testing. This wasn't a logic bug; InMemory has no concept of an atomic conditional SQL `UPDATE`, so testing atomicity against it would never have proven anything real even if it happened to pass. **Fix:** `WorkflowRunClaimServiceTests` now runs against a real, disposable Postgres database instead (new `PostgresTestDatabase.cs` — creates a uniquely-named database per test against the local Docker Postgres from PD-3, port 5433, and drops it afterward). **New prerequisite as a result: Docker must be running locally (`docker compose up -d`) for this test class to pass** — a deliberate trade-off given what's actually being tested, not an oversight. The state-machine tests from PD-4 are unaffected and still run against InMemory, since they don't depend on provider-specific atomicity. **Needs Brock to run `docker compose up -d`, then `dotnet build`, then `dotnet test`, and report back.** |
