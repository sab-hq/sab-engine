...
*As of Aug 10, 2026: ongoing Phase 1 build progress is now tracked in [`CHANGELOG.md`](../../CHANGELOG.md) (short, scannable, one entry per PD item) rather than continuing to grow the dense paragraph-per-version entries below. This section stays for the document's own version history — major design revisions, not individual PD build items going forward.*

- **v0.1.2** (Aug 4–6, 2026) — Added RMM/PSA/MCP connector research to Section 6 (specific integration priorities: NinjaOne, ConnectWise Manage, Autotask PSA, Ansible Automation Platform/AWX, Hudu, plus partner-program notes and a phased rollout) and to Section 8 (2026 MCP ecosystem findings, including documented security risks that reinforce the recommend-and-approve principle). Also closed out most remaining "to be filled in" placeholders across Sections 2–9 with concrete first-draft detail (state machine, module metadata schema, connector interface, Engine State Store data model, API surface) — items that genuinely need Brock's input (revenue share %, compliance framework target, target dates) are explicitly flagged rather than invented, and collected in Section 10. Reformatted headers throughout so the label and its following text are visually separated, not run together. Confirmed Docker containers as the module sandboxing mechanism (SE-2). Confirmed every module and every workflow requires a unique ID (new AR-5); reflected in Sections 3 and 4.2. Confirmed "Open Source Module Library (OSML)" as the official name for the module library / `sab-hq/sab-modules` repo (new AR-6); reflected in Sections 4.2, 5.1, and 9. Confirmed "ESS" as the official shorthand for the Engine State Store (new AR-7); reflected in Section 4.5. Added a full set of plain-language companion docs (`what-is-sab.md` through `open-source-module-library.md`) and cross-linked them throughout this document. Added `pre-development-checklist.md`, a dependency-ordered pre-code task list (PD-1 through PD-29). **Corrected Section 9's Sequencing Note**, which had overstated — on this document's own initiative, not Brock's instruction — that `sab-kb` must ship strictly before `sab-engine`'s Phase 1; the actual, twice-confirmed resolution (RC-5 originally, PD-1 again this session) is that the two are worked on in parallel. **Phase 1 development began (Aug 6):** real C#/.NET code now exists and is verified — the solution scaffold, the PostgreSQL/EF Core schema, and a working, tested state machine with hash-linked audit logging (PD-1 through PD-4 and PD-8, all done and confirmed by Brock). This is the first point where `sab-engine` has actual code, not just design. **Concurrency (PD-5) is also done** — multiple workers can now safely claim work from the same queue via an atomic Postgres `UPDATE ... WHERE`, verified with 20/20 tests passing. This surfaced the project's first real testing-strategy correction: EF Core's InMemory provider can't translate the atomic update this depends on, so those specific tests now run against a real, disposable-per-test Postgres database — meaning **Docker must be running locally for `dotnet test` to fully pass from here on**, not just for the PD-3 migration step. **The AI agent layer (PD-6) is also done** — real Semantic Kernel integration, with Section 4.1/4.2's hard rule (never propose a module that isn't `production-approved` or lacks a tested rollback) enforced directly in the agent itself, as defense in depth on top of the orchestration engine's own enforcement. Verified with 27/27 tests passing, using a hand-written fake chat-completion service so the plan-parsing/validation logic is tested without any real API key or network call. Deliberately not done yet: wiring to a real model (needs a real credential from Brock, not something to hardcode) and a real module catalog loader (no real modules exist yet — PD-14–PD-17). **PowerShell interop (PD-7) is also done** — `PowerShellExecutor` runs a script locally via `Microsoft.PowerShell.SDK` and returns a structured result. Verified with 32/32 tests passing; two real issues found and fixed along the way: a `Microsoft.CodeAnalysis` version conflict between `Microsoft.PowerShell.SDK` and `Microsoft.EntityFrameworkCore.Design`'s transitive dependencies (resolved via explicit pins in `SabEngine.Api.csproj`), and a genuine logic gap where PowerShell's `throw` (a terminating error, propagated as a real .NET exception) wasn't being caught the way `Write-Error` (a non-terminating error) already was — fixed with a try/catch that folds both into the same structured result. Deliberately scoped to local execution only; the actual WinRM connector (PD-23–PD-27) that points this interop at a remote server was still ahead at the time — done now, see below. **The secrets backend (PD-9) is also done** — per SE-1's own direction (native OS credential store as the simpler Phase 1 default), implemented against Windows Credential Manager via hand-written P/Invoke, behind the pluggable `ISecretStore` contract in Core so a Vault-backed implementation can swap in later without touching anything that calls it. This was flagged in advance as the highest-risk code in the project so far (struct marshaling bugs fail at runtime, not compile time) and verified clean on the first attempt — 38/38 tests passing, including a Unicode round-trip. **A CI pipeline (PD-10) is also done** — `.github/workflows/ci.yml`, two jobs split by OS to satisfy two conflicting infrastructure needs at once (the Windows-only Credential Manager tests vs. Postgres, which is more reliably supported on Linux runners in GitHub Actions). Verified green on the actual GitHub Actions runners on the first push — the first PD item that couldn't be checked with a local `dotnet` command at all. **PD-11's lab environment is also done** — `sabengine-labwin01`, an Azure Windows Server 2022 VM in Central US, confirmed reachable via RDP and properly deallocated between sessions with a $10/month budget alert in place. The plan changed during setup: B-series (including the originally planned B1s) returned `NotAvailableForSubscription` on Brock's subscription in both East US and Central US — a quota gap, not a regional one — so D2s_v3 was used instead, at a confirmed $0.2110/hr (~$154/month if ever left running continuously, versus B1s's ~$15–$20/month). This makes the deallocate-between-sessions discipline meaningfully more important than the original plan assumed. One flagged, not-yet-acted-on tradeoff: the image is Hotpatch-enabled, which may not exercise sab-engine's actual reboot-handling path the way a non-Hotpatch image would — worth a second VM later specifically for that testing. **The module manifest parser (PD-12) is also done** — new project `SabEngine.Modules` reads and validates the Section 4.2 YAML manifest format, enforcing every required field at parse time and closing a loop PD-6 left open (a real parsed manifest now feeds `SabAgent.ProposePlanAsync` directly via a new `ToModuleCandidate()` bridge). **Verification caught a genuine bug in this document itself**, not in the code: Section 4.2's own literal YAML example, `{ type: enum[success, failure] }`, isn't actually valid YAML inside a flow mapping (`[`, `]`, and `,` are structural characters there) — corrected in this document to `{ type: "enum[success, failure]" }`. **PD-13 (a CI pipeline for `sab-modules`) and PD-14 (the first real module, `pre-flight-check`) are both also done**, verified together: a new `SabEngine.Modules.Cli` project wraps PD-12's parser as a validator, `sab-modules`' own CI checks out `sab-engine` and runs it on every push (a deliberate Phase 1 simplification over publishing a NuGet tool — documented in `pre-development-checklist.md`, PD-13), and running it locally confirmed `pre-flight-check`'s manifest parses correctly, closing the exact verification gap PD-14 had flagged as open. `pre-flight-check` itself (real disk-space/pending-reboot/service-status checks, a deliberate no-op rollback since it only reads, and 8 passing Pester tests with every external check mocked) is the first genuinely working SAB module, in the `sab-modules` repo. **PD-15 (`stage-patches`, the second module) is also done** — downloads (never installs) available Windows updates via the native Windows Update Agent COM API, verified with 5/5 Pester tests passing and both manifests now confirmed valid via the CLI validator (`2/2 manifest(s) valid`). This was the hardest debugging session so far: two genuinely subtle bugs, both worked through with a real diagnostic trail rather than guessed at — Pester v6 not reliably carrying top-level helper functions into test scope (fixed by moving them into `BeforeAll`), and a classic PowerShell gotcha where an *empty* `ArrayList` returned from a mocked function silently unrolled to zero pipeline output items (since `ArrayList` implements `IEnumerable`), making an assignment end up `$null` instead of the empty collection — fixed with `Write-Output -NoEnumerate`. **PD-16 (`apply-patches`, the third module) and PD-18 (rollback procedures for `stage-patches` and `apply-patches`) are both also done** — `apply-patches` is the first module with `risk_level: high`, the only one that actually changes a server's running state, and the first with a genuine, non-trivial rollback (`wusa.exe /uninstall`, actually reversing an installed patch) rather than a justified no-op. Applying the `-NoEnumerate` lesson from PD-15 proactively meant zero repeat bugs this time — verified clean on the first attempt, 10/10 Pester tests passing, all three manifests confirmed valid (`3/3 manifest(s) valid`). PD-18 needed no separate work — both modules' rollbacks were already built as part of PD-15/PD-16, so it's tracked as closed via cross-reference rather than duplicated. **PD-17 (`validate`, the fourth and final module), PD-19 (unique IDs/`lab-validated` status for all four modules), and PD-20 (each module's `tests.lab_suite` file) are all also done** — `validate` closes the loop `pre-flight-check` opened, confirming both that specific patches actually landed (a WUA search with `IsInstalled=1`) and that the server is still healthy afterward, deliberately excluding the pending-reboot check since that's expected right after patching, not a failure. Verified clean on the first attempt, 6/6 Pester tests passing, all four manifests confirmed valid (`4/4 manifest(s) valid`). PD-19 and PD-20 needed no separate work either — both were satisfied as each module's manifest and test file were written, not deferred. **All four patching modules now exist, tested (29 tests total across the set), and verified** — the next step is stringing them together into the actual "Patch Windows Server" workflow (PD-21). **PD-21 (the workflow definition) and PD-22 (its unique ID) are also done**, with a real, honestly-flagged gap: unlike modules, Section 4.2 never gave a concrete YAML example for workflows, so there's no formal schema or automated validator for this file the way `SabEngine.Modules`/`SabEngine.Modules.Cli` validate module manifests — building that is real future work, not itemized yet. **PD-23 (the WinRM connector) and PD-24 (credential resolution) are also done, for everything testable.** Building the first real `IExecutionConnector` implementation surfaced a genuine bug in that interface itself, from PD-2: `ExecuteAsync` had no way to receive a `WorkflowRunId`, even though its own return type requires one — fixed at the interface level. Verified with 55/55 tests passing, using an injectable seam that substitutes a real local PowerShell session for the real remote WinRM connection, since the latter can't be meaningfully mocked. **The one thing that remains genuinely unverified: the actual network connection to a real target** — confirming that needs a deliberate, real-cost step (starting the lab VM and checking WinRM specifically, not just RDP) that hasn't happened yet. `pre-development-checklist.md` and `sab-engine-overview.md` are the up-to-date sources for exact build status — not duplicated here. **This design document itself moved into `docs/design/`** (Aug 6) — `open-questions.md` and `pre-development-checklist.md` stay at `docs/` root; the two phase docs live at `docs/phases/`; and the full plain-language companion series, now including `sab-engine-overview.md` too, lives in `docs/learn/`. Every link in this file to a sibling doc has been updated to `../learn/filename.md` for the companion docs, and `../filename.md` or `../phases/filename.md` for everything else, accordingly.
