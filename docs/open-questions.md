...
| AR-6 | What is the official name for the open source module/workflow repository (`sab-hq/sab-modules`)? | 🟢 Resolved — **the Open Source Module Library, or "OSML" for short, confirmed by Brock (until a new name is adopted).** This is the same thing previously referred to informally as "the module library" — Section 4.2's conceptual system (the module contract, cataloguing rules) and the `sab-hq/sab-modules` repo it's implemented in. "OSML" is now the standard term across the design doc and the beginner doc set (`open-source-module-library.md`).
| AR-7 | What is the official shorthand for the Engine State Store (Section 4.5)? | 🟢 Resolved — **"ESS," confirmed by Brock (until a new name is adopted).** Same pattern as AR-6/OSML — "Engine State Store" stays the full name, "ESS" is now the standard shorthand across the design doc and the beginner doc set (`engine-state-store.md`).
