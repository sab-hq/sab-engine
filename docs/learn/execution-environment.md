...
> **Current status:** the WinRM connector this doc describes is now real, working code as of PD-23/PD-24 in `pre-development-checklist.md` — it resolves a credential handle against Windows Credential Manager, opens a remote PowerShell session, runs a real module script against it, and returns a structured result. Least-privilege credential scoping is real now too (PD-25) — a resolver that picks a tier-specific credential (`"target:elevated"` vs. a plain `"target"` fallback) instead of one standing credential for everything. **Per-target isolation is also real now (PD-26)** — a hang against one target gets forcibly interrupted after a configurable timeout (PowerShell's `Stop()`, used here for the first time in the project) rather than blocking forever, and runs on its own dedicated thread so it can never starve resources another target's execution needs. Proven directly with a real concurrent test: a hanging execution against one target and a fast one against another, confirming the fast one is never delayed. **Docker-based sandboxing is real now too (PD-27), with an important, honest limitation.** A `DockerSandboxedExecutor` runs a script inside a disposable, network-isolated, read-only-mounted container — both isolation guarantees directly tested, not just assumed. But SAB's actual four modules are Windows-specific (WUA COM APIs, `Get-Service`, `wusa.exe`), and none of that exists inside the standard Linux container this defaults to; sandboxing the real modules would need a Windows container image, which means switching Docker Desktop to Windows-container mode — a genuinely disruptive step, deliberately not forced silently. So this sandboxes *any* PowerShell script correctly, but doesn't yet sandbox SAB's own real modules end to end. 66/66 tests total. **One honest, important gap remains:** the tests substitute a real *local* PowerShell session for the real remote connection, since a genuine WinRM connection can't be meaningfully faked in a test — so the actual network connection to a real server has never been tried yet. Confirming that for real means deliberately starting the lab VM (PD-11) and checking WinRM is reachable there, which hasn't happened. **Also worth knowing:** PD-25's resolver exists, but nothing calls it per-module yet — that wiring depends on orchestration work that doesn't exist yet, now tracked as PD-31 in `checklist-02.md`.
