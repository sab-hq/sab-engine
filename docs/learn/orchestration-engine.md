...
> **Current status:** the state machine and audit trail described in this doc are real, working code as of PD-4/PD-8 in `pre-development-checklist.md` — not just a plan. The claim/lease concurrency pattern (PD-5) is also done — more than one worker can now safely pull from the queue without racing each other. **Sequencing itself (actually calling modules in order) is not built yet** — real design work on exactly how it should behave has happened though, tracked in `checklist-02.md` (PD-32 through PD-36): crash recovery is designed around reconnecting and assessing the target's real state before deciding anything, mirroring how an actual systems engineer would handle an interruption, rather than a reflexive rollback or a blind resume. **One honest caveat on the "A useful mental model" section below:** it describes the *intended* design, not yet-verified behavior — in the code that exists today, a run stuck in `Executing` (say, from a crashed worker) currently has no path back to being reclaimed at all; that's a known, tracked gap (PD-33), not yet closed.
